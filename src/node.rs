...
use crate::{Args, IncomingRequest, ResponseConnection};
...
impl Node {
    ...
    pub async fn connect(&self) {
        ...
        let request_payload = IncomingRequest::Connection {
        ...
        let response_connection = match res.json::<ResponseConnection>().await {
    ...
    pub async fn process_connection(&self, address: String) -> ResponseConnection {
        let mut connected_nodes = self.connected_nodes.lock().unwrap();

        let response = ResponseConnection {
